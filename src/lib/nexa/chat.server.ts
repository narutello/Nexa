...
import { assertCanUseChat, assertConversationAccess, assertNotSelfTarget, toActor } from "./authz";
import { NexaError } from "./errors";
import { isUuid, sanitizeBody } from "./evidence";
import { ensureProfile, presenceForPeers, toPublic, touchPresence } from "./profile.server";
...
import { INBOX_PAGE, MESSAGE_MAX_LENGTH, MESSAGE_PAGE, TYPING_TTL_MS } from "./types";

function pairKey(a: string, b: string): string {
  return a < b ? `${a}:${b}` : `${b}:${a}`;
}

async function requireMember(sql: Sql, conversationId: string, userId: string): Promise<void> {
...
async function otherMemberId(sql: Sql, conversationId: string, userId: string): Promise<string> {
  const rows = await sql.query<{ user_id: string }>(
    "select user_id from conversation_members where conversation_id = $1 and user_id <> $2",
    [conversationId, userId],
  );
...
async function loadInbox(
  ...
  params.push(limit);
  const rows = await sql.query<ConvoRow>(
  ...
  const lastMsgs = await sql.query<{
  ...
  const unread = await sql.query<{ conversation_id: string; unread: number }>(
  ...
  const typing = await sql.query<{ conversation_id: string }>(
    `select conversation_id from typing_indicators
     where conversation_id in (${idPlaceholders})
  ...
  return rows.map((r) => {
    ...
    const other: PublicProfile = toPublic(
      {
        user_id: r.other_id,
        ...
        role: "USER",
        status: (r.status as "active") ?? "active",
        status_reason: null,
...
export async function listConversationsImpl(
  userId: string,
  cursor: string | null,
): Promise<{ items: ConversationPreview[]; nextCursor: string | null }> {
  const sql = await getSql();
  const me = await ensureProfile(sql, userId);
  assertCanUseChat(toActor(me));
  await touchPresence(sql, userId);
  const items = await loadInbox(sql, userId, null, INBOX_PAGE);
  // cursor is for older pages (last_message_at < cursor)
  if (cursor) {
    const older = await sql.query<ConvoRow>(
      `select c.id, c.last_message_at,
              p.user_id as other_id, p.display_name, p.handle, p.avatar_hue, p.last_seen_at, p.status,
              cm.last_read_at
       from conversation_members cm
       ...
       order by c.last_message_at desc
       limit $3`,
      [userId, cursor, INBOX_PAGE],
    );
    const mapped = await loadInboxFromRows(sql, userId, older);
    const nextCursor =
      mapped.length === INBOX_PAGE ? mapped[mapped.length - 1]?.lastMessageAt ?? null : null;
    return { items: mapped, nextCursor };
  }
  const nextCursor =
    items.length === INBOX_PAGE ? items[items.length - 1]?.lastMessageAt ?? null : null;
  return { items, nextCursor };
}

async function loadInboxFromRows(
  sql: Sql,
  userId: string,
  rows: ConvoRow[],
): Promise<ConversationPreview[]> {
...
export async function startConversationImpl(
  userId: string,
  otherUserId: string,
): Promise<{ id: string; other: PublicProfile }> {
  const sql = await getSql();
  ...
  if (!other[0] || other[0].status === "banned") {
    throw new NexaError("User not found.", 404, "NOT_FOUND");
  }
  ...
  if (!id) {
    ...
    await sql.query(
      "insert into conversations (id, pair_key) values ($1, $2) on conflict (pair_key) do nothing",
      [id, key],
    );
  ...
  return { id, other: toPublic(other[0]) };
}
...
export async function getConversationImpl(
  ...
  return { id: conversationId, other: toPublic(other[0]) };
}
...
export async function listMessagesImpl(
  ...
  const slice = hasMore ? rows.slice(0, MESSAGE_PAGE) : rows;
  const items = slice.map(mapMessage).reverse();
  const nextCursor = hasMore ? toIso(slice[slice.length - 1]?.created_at) : null;
  return { items, nextCursor };
}

export async function sendMessageImpl(
  userId: string,
  data: { conversationId: string; body: string; clientId?: string },
): Promise<ChatMessage> {
  const sql = await getSql();
  ...
  const existing = await sql.query<{
    ...
    created_at: string | Date;
    deleted_at: string | Date | null;
  }>("select id, conversation_id, sender_id, body, created_at, deleted_at from messages where id = $1", [
    id,
  ]);
  if (existing[0]) {
    if (existing[0].sender_id !== userId) throw new NexaError("Invalid message id.", 400);
    return mapMessage({ ...existing[0], delivered_at: null, read_at: null });
  }

  await sql.query(
    `insert into messages (id, conversation_id, sender_id, body)
     values ($1, $2, $3, $4)`,
    [id, data.conversationId, userId, body],
  );
  await sql.query(
  ...
  return mapMessage({ ...row[0]!, delivered_at: null, read_at: null });
}
...
export async function markReadImpl(
  ...
  const owned = await sql.query<{ id: string; created_at: string | Date }>(
  ...
  await sql.query(
    ...
    [conversationId, userId, owned[0].created_at, targetId],
  );
  await sql.query(
    `update message_receipts r
...
export async function setTypingImpl(
  userId: string,
  conversationId: string,
  typing: boolean,
): Promise<void> {
...
export async function syncImpl(
  ...
  const payload: SyncPayload = {
    ...
    me: { role: me.role, status: me.status, statusReason: me.status_reason },
    inbox: [],
    messages: [],
    receipts: [],
    presence: [],
  ...
  payload.inbox = await loadInbox(sql, userId, data.inboxAfter, INBOX_PAGE);
  ...
  if (data.conversationId) {
    ...
    if (member[0]) {
      const params: unknown[] = [data.conversationId];
      let afterClause = "";
      ...
      payload.messages = rows.map(mapMessage);
      ...
      const receipts = await sql.query<{
        ...
        `select r.message_id, r.delivered_at, r.read_at
         ...
         where m.conversation_id = $1
           and m.sender_id = $2
           and (r.delivered_at is not null or r.read_at is not null)`,
        [data.conversationId, userId],
      );
      payload.receipts = receipts.map(
        (r): ReceiptUpdate => ({
      ...
      const typing = await sql.query<{ user_id: string }>(
        `select user_id from typing_indicators
         where conversation_id = $1 and user_id <> $2 and expires_at > now()`,
        [data.conversationId, userId],
      );
      payload.typingUserIds = typing.map((t) => t.user_id);
    }
  }

  const peerIds = [
    ...data.peerIds,
    ...payload.inbox.map((c) => c.other.userId),
  ];
  payload.presence = await presenceForPeers(sql, userId, peerIds);
  return payload;
}

function assertCanUseChatSafe(status: string): boolean {
  return status === "active" || status === "warned";
}

export async function loadMessagesForEvidence(
  sql: Sql,
  conversationId: string,
  userId: string,
): Promise<{ id: string; sender_id: string; body: string; created_at: string | Date }[]> {
  await requireMember(sql, conversationId, userId);
  return sql.query(
